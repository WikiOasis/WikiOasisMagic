 ...
 *
 * The in-flight transaction is tracked in static properties rather than instance ones
 * because {@see \MediaWiki\JobQueue\JobQueueGroup::get()} builds a brand new queue
 * object on every call: the instance that pops a job is never the instance that
 * acknowledges it. Instance state would therefore always be empty in {@see doAck()},
 * leaving every transaction to be closed by the shutdown finalizer and reported as a
 * failure. Only one job is processed at a time per process, so a single slot suffices.
 *
...
class JobQueueSentryRedis extends JobQueueRedis {
	...
	private static $pendingProcessTransaction = null;

	/** @var string|null UUID of the job owning {@see $pendingProcessTransaction}. */
	private static $pendingProcessUuid = null;

	/** @var Span|null The span that was active on the hub before we started processing. */
	private static $previousSpan = null;

	/** @var bool Whether a shutdown finalizer has been registered for this process. */
	...
	protected function doPop() {
		...
		self::finishPendingProcessSpan( false );
	...
	protected function doAck( RunnableJob $job ) {
		...
		if ( self::$pendingProcessTransaction !== null
			&& (string)$job->getMetadata( 'uuid' ) === self::$pendingProcessUuid
		) {
			$error = $job->getLastError();
			$ok = ( $error === null || $error === '' );
			self::finishPendingProcessSpan( $ok );
		}
	...
	private function startProcessSpan( RunnableJob $job ): void {
		...
		$hub = SentrySdk::getCurrentHub();
		self::$previousSpan = $hub->getSpan();
		$hub->setSpan( $transaction );

		self::$pendingProcessTransaction = $transaction;
		self::$pendingProcessUuid = (string)$job->getMetadata( 'uuid' );

		self::registerShutdownFinalizer();
	}
	 ...
	 */
	private static function finishPendingProcessSpan( bool $ok ): void {
		if ( self::$pendingProcessTransaction === null ) {
			return;
		}

		$transaction = self::$pendingProcessTransaction;
		$transaction->setStatus( $ok ? SpanStatus::ok() : SpanStatus::internalError() );
		$transaction->finish();

		SentrySdk::getCurrentHub()->setSpan( self::$previousSpan );

		self::$pendingProcessTransaction = null;
		self::$pendingProcessUuid = null;
		self::$previousSpan = null;
	}
	 ...
	 */
	private static function registerShutdownFinalizer(): void {
